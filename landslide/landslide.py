...
HISTORY_CACHE_PATH = os.environ.get(
    "LANDSLIDE_HISTORY_CACHE",
    os.path.join(SOURCE_DIR, "landslide_history.json"),
)
OUTPUT_HTML = os.environ.get(
    "OUTPUT_HTML",
    os.path.join(SCRIPT_DIR, "landslide_similarity.html"),
)
...
REGION = os.environ.get("REGION", "경기도")
REGION_NAME_PATTERNS = REGION_PRESETS.get(REGION, [REGION])

LANDSLIDE_API_URL = (
    "https://www.safetydata.go.kr/V2/api/DSSP-IF-00134"
)
SERVICE_KEY = os.environ.get("LANDSLIDE_SERVICE_KEY")
if not SERVICE_KEY:
    raise ValueError("LANDSLIDE_SERVICE_KEY is missing. Set it in .env or your shell environment.")
PAGE_SIZE = int(os.environ.get("LANDSLIDE_PAGE_SIZE", "1000"))
...
# -------------------------------------------------
# Fetch every page of landslide history (cached)
# -------------------------------------------------
def fetch_landslide_history():
    if os.path.exists(HISTORY_CACHE_PATH):
        with open(HISTORY_CACHE_PATH, "r", encoding="utf-8") as f:
            cached = json.load(f)
        print(f"Loaded cached landslide history: {len(cached)} records")
        return cached
    ...
    while True:
        ...
        resp = session.get(LANDSLIDE_API_URL, params=params, timeout=60)
        ...
        if total_count is None:
            ...
            print(f"Landslide history totalCount: {total_count}")
...
def reference_points_within(region_geom, lonlat_records):
    ...
    if not candidate_features:
        raise ValueError("No landslide history points fall within the region.")
...
# Pipeline
# -------------------------------------------------
records = fetch_landslide_history()
...
Map.addLayer(
    ...
    "AlphaEarth landslide similarity",
)
Map.addLayer(
...
Map.addLayer(
    reference_fc,
    {"color": "#542788"},
    "Past landslide points (reference)",
)
Map.addLayerControl()
